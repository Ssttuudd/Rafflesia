...
#define SKIP_8(stream)				{ uint8_t ukn; stream >> ukn;	}
#define SKIP_16(stream)				{ uint16_t ukn; stream >> ukn;	}
#define SKIP_32(stream)				{ uint32_t ukn; stream >> ukn;	}
#define SKIP_N(stream, n)			{ char ukn[n]; stream >> ukn;	}
#define SKIP_N_RUNTIME(stream, n)	{ std::vector<char> ukn(n); stream.read(ukn.data(), n); }
...
namespace simple
{
	...
	public:
		...
		{
			std::fseek(input_file_ptr, position, SEEK_SET);
	...
	template<typename same_endian_type>
	file_istream<same_endian_type>& operator >> (file_istream<same_endian_type>& istm, std::wstring& val)
	{
		val.clear();
	...
	public:
		...
		bool seekg(size_t position)
		{
			if (position < m_vec.size())
				m_index = position;
			else
				return false;
		...
		void read(bool t)
		{
			...
			std::memcpy(reinterpret_cast<void*>(&t), &m_vec[m_index], 1);
		...
		void read(wchar_t* p, size_t size)
		{
		...
		void read(std::wstring& str, const unsigned int size)
		{
			if (eof())
				throw std::runtime_error("Premature end of array!");
	...
	template<typename same_endian_type, typename T>
	mem_istream<same_endian_type>& operator >> (mem_istream<same_endian_type>& istm, bool val)
	{
		istm.read(val);
	...
	template<typename same_endian_type>
	mem_istream<same_endian_type>& operator >> (mem_istream<same_endian_type>& istm, std::wstring& val)
	{
		val.clear();
	...
	template<typename same_endian_type>
	ptr_istream<same_endian_type>& operator >> (ptr_istream<same_endian_type>& istm, std::wstring& val)
	{
		val.clear();
	...
	template<typename same_endian_type>
	memfile_istream<same_endian_type>& operator >> (memfile_istream<same_endian_type>& istm, std::wstring& val)
	{
		val.clear();
	...
	template<typename same_endian_type>
	file_ostream<same_endian_type>& operator << (file_ostream<same_endian_type>& ostm, const std::wstring& val)
	{
	...
	public:
		...
		std::uint16_t swapEndian(std::uint16_t u)
		{
			return (u >> 8) | ((u & 0xFF) << 8);
		}

		void write(const wchar_t* p, size_t size)
		{
			for (size_t i = 0; i < size; ++i)
			{
				wchar_t swapped = p[i];
				m_vec.push_back(((char*)&swapped)[0]);
				m_vec.push_back(((char*)&swapped)[1]);
			}
			m_vec.push_back('\0');
			m_vec.push_back('\0');
		}
		...
		template<typename T>
		void writeat(size_t position, const T& t)
		{
			std::vector<char> vec(sizeof(T));
			T t2 = t;
			simple::swap_endian_if_same_endian_is_false(t2, m_same_type);
			std::memcpy(reinterpret_cast<void*>(&vec[0]), reinterpret_cast<const void*>(&t2), sizeof(T));
			writeat(position, vec);
		}

		template<typename T>
		void writeat(size_t position, bool t)
		{
			std::vector<char> vec(sizeof(T));
			T t2 = t;
			simple::swap_endian_if_same_endian_is_false(t2, m_same_type);
		...
		void writeat(size_t position, const std::vector<char>& vec)
		{
			for (size_t n = 0, count = vec.size(); n < count; n++)
				m_vec[position++] = vec[n];
		}
	private:
		std::vector<char> m_vec;
		same_endian_type m_same_type;
	...
	template<typename same_endian_type>
	mem_ostream<same_endian_type>& operator << (mem_ostream<same_endian_type>& ostm, const std::wstring& val)
	{
		ostm.write(val.c_str(), val.size());
	...
	template<typename same_endian_type>
	memfile_ostream<same_endian_type>& operator << (memfile_ostream<same_endian_type>& ostm, const std::wstring& val)
	{
		uint8_t size = val.size() * sizeof(wchar_t);
		ostm.write(size);
